...
-(id)initWithIndex:(int)i andData:(NSDictionary*)data {
    ...
    if (self !=nil) {
       ...
       // NSLog(@"%@",data);
        caption.text = [NSString stringWithFormat:@"@%@ (%@)",[data objectForKey:@"username"],[data objectForKey:@"points"]];
        [self addSubview: caption];
		//add touch event
		[self addTarget:delegate action:@selector(didSelectArenaPhoto:) forControlEvents:UIControlEventTouchUpInside];
		//load the image
