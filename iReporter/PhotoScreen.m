...
-(void)viewDidLoad {
   ...
   // self.navigationItem.title = @"Post photo";
    //self.navigationItem.title = IdTagPS;
...
-(IBAction)btnActionTapped:(id)sender {
	...
	//show the app menu
	[[[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"Close" destructiveButtonTitle:nil otherButtonTitles:@"Take photo", @"Effects!", @"Post Photo", @"Logout", nil] showFromTabBar:self.tabBarController.tabBar];
	 //show:self.view];
...
-(void)uploadPhoto {
    ...
    [[API sharedInstance] commandWithParams:[NSMutableDictionary dictionaryWithObjectsAndKeys:@"upload", @"command", UIImageJPEGRepresentation(photo.image,70), @"file", fldTitle.text, @"title",IdTagPS,@"tagID", nil] onCompletion:^(NSDictionary *json) {
		//completion
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    UIImage *scaledImage = [image resizedImageWithContentMode:UIViewContentModeScaleAspectFill bounds:CGSizeMake(photo.frame.size.width, photo.frame.size.height) interpolationQuality:kCGInterpolationMedium
